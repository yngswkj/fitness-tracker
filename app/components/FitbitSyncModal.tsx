...
export default function FitbitSyncModal() {
    ...
    const executeSync = async (from: string, to: string) => {
        ...
        try {
            const response = await fetch('/api/fitbit/sync-range', {
                method: 'POST',
            ...
            if (response.ok) {
                setSyncResult({
                    ...
                    rateLimitError: result.summary?.rateLimitHit || false
                })
            } else {
                setSyncResult({
                    success: false,
                    message: result.error || '同期中にエラーが発生しました',
                    rateLimitError: result.rateLimitError || false
                })
            }
        } catch (error) {
                        ...
                        <div className="flex items-start gap-2">
                            ...
                            <div className="text-sm text-blue-800">
                                ...
                                <ul className="list-disc list-inside space-y-1 text-xs">
                                    <li>未取得またはstepsがNULLの日付を対象に同期</li>
                                    <li>前日までのデータのみ取得可能</li>
                                    <li>レート制限時は処理を中断し、取得済みデータを保存</li>
                                </ul>
                            </div>
                        </div>
                    </div>
